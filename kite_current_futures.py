#!/usr/bin/env python3
"""
Kite Current Futures Data Fetcher
Specialized for current month futures (0-35 days expiry) with ultra-fast performance
"""
...
class KiteCurrentFutures:
    ...
    def get_current_futures_contracts(self, exchange="NFO"):
        """Get current month futures contracts (0-35 days expiry)"""
        try:
            if self.current_contracts:
                return self.current_contracts
            
            print(f"🔍 Fetching CURRENT futures contracts from exchange: {exchange}")
                ...
                try:
                    ...
                    if (instrument_type == 'FUT' and 
                        ...
                        try:
                            ...
                            # Only current month futures (0-35 days)
                            if 0 <= days_diff <= 35:
                                contract = {
    ...
    def fetch_live_data(self, use_ltp_only=True, limit_contracts=None):
        """Fetch live data for current futures"""
        try:
            contracts = self.get_current_futures_contracts()
            ...
            print(f"📈 Fetching live data for {len(contracts)} current futures...")
    ...
    try:
        ...
        while True:
            try:
                ...
                current_futures.fetch_live_data(use_ltp_only=False, limit_contracts=None)
                current_futures.display_live_data(limit=None)
